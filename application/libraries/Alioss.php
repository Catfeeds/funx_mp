...
class Alioss {
	...
	public function do_upload($field = '')
	{
		...
		//此处替换为将文件上传到阿里云的OSS服务中
		$CI =& get_instance();
		$CI->config->load('alioss', TRUE);
		
		$accessKeyId = $CI->config->item('alioss')['accessKeyId'];
		$accessKeySecret = $CI->config->item('alioss')['accessKeySecret'];
		$endpoint = $CI->config->item('alioss')['endpoint'];
		$bucket = $CI->config->item('alioss')['bucket'];
		...
		try{
			...
			// printf($e->getMessage() . "\n");
			$this->set_error('upload_destination_error '.$e->getMessage(), 'error');
			return FALSE;
		}
		$this->oss_path='/'.$object;
