...
class Serviceorder extends MY_Controller
{
    ...
    public function Order()
    {
        ...
        $listorder = Serviceordermodel::with('roomunion')->where('uxid',$this->current_id)->whereIn('status',["SUBMITTED","PENDING","PAID","SERVING"])->orderBy('id','desc')->get($field);
        foreach ($listorder as $key=>$value){
            $listorder[$key]['paths'] = $this->fullAliossUrl($value['paths']);
        }
        $listordered = Serviceordermodel::with('roomunion')->where('uxid',$this->current_id)->whereIn('status',["COMPLETED","CANCELED"])->orderBy('id','desc')->get($field);
