...
class Draw extends MY_Controller
{
    ...
    public function drawQualifications()
    {
        ...
        if ($Qualifications == 1) {
            ...
            $resident = Residentmodel::where(['id' => CURRENT_ID, 'status' => 'NORMAL'])->get();
            if (!$resident) {
                $this->api_res(11004);
                return false;
            }
        } elseif ($Qualifications == 3) {
            $resident = Residentmodel::where(['id' => CURRENT_ID, 'status' => 'NORMAL_REFUND'])->get();
            if (!$resident) {
                $this->api_res(11004);
                return false;
            }
